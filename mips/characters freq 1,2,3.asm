# calculate the freq of each character in the paragrah 
		.data
...
input:		
		...
		addi $v0,$v0, -65 # Get the input symbols so that A=0, B=1, etc...)
array_op:
...
lowercase:
		addi $v0, $v0,-97 # Get the input symbols so that a=0, b=1, etc...)
...
print_loop:	
		...
		addi $s0, $s0,1 # Increment the letter priting variable by one, so that we print the next letter
		...
		beqz $a0, print_loop #if frequncy of char is zero skip char and contniue loop
		#beq $a0, $zero print_loop
		bgt $a0, 3, print_loop #if frequncy of char is greater than 3 skip char and contniue loop
